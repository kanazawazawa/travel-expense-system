...
**重要**: 音声チャット機能は **Blazor Server のアーキテクチャにより、API Key は安全に保護されています**。
...
│  ┌──────────────────────────────────────────────────┐  │
│  │    VoiceChatComponent.razor (C#)                 │  │
│  │    - SignalRで音声データを受信                   │  │
│  │    - DotNetObjectReference経由で呼び出し         │  │
│  └──────────────┬───────────────────────────────────┘  │
│                 │                                       │
│  ┌──────────────┴───────────────────────────────────┐  │
│  │    RealtimeAudioService.cs (C#)                  │  │
│  │    - ClientWebSocket で接続                      │  │
│  │    - API Key設定 (サーバー側メモリのみ)         │  │
│  └──────────────┬───────────────────────────────────┘  │
│                 │                                       │
│                 │ ② 素のWebSocket (ClientWebSocket)    │
│                 │    wss://xxx.cognitiveservices.azure.com/openai/realtime
│                 │    Headers: api-key (サーバー側のみ)  │
└─────────────────┼───────────────────────────────────────┘
                  ↓
┌─────────────────┴───────────────────────────────────────┐
│        Azure OpenAI Realtime API                        │
│        wss://xxx.cognitiveservices.azure.com            │
└─────────────────────────────────────────────────────────┘
```
...
| 通信 | プロトコル | 用途 | セキュリティ |
|------|-----------|------|-------------|
| **① ブラウザ ↔ Blazor Server** | **SignalR** (WebSocket) | UI通信、イベント処理、音声データ送信 | ✅ API Key含まれない |
| **② Blazor Server ↔ Azure OpenAI** | **素のWebSocket** (ClientWebSocket) | 音声データ送受信、AI応答 | ✅ API Key はサーバー側のみ |

**セキュリティのポイント**:
- ✅ **API Key はブラウザに露出しません**（Blazor Server で実行されるため）
- ✅ ブラウザから見えるのは自分のサーバーへのSignalR接続のみ
- ✅ Azure OpenAIへの接続はサーバー側で確立（ブラウザからは見えない）
- ✅ Networkタブで `cognitiveservices.azure.com` への直接接続は表示されない

**重要な注意点**:
- この実装は **Blazor Server** です（C#コードはサーバー側で実行）
- **Blazor WebAssembly** では同じコードでもブラウザで実行されるため、API Keyが露出します
- 本番環境では **Managed Identity** の使用を推奨（API Keyの代わり）
